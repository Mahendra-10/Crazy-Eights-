...
public class User extends Player {
   ...
   public Card makeMove(Game crazyEight, Card top) {
      ...
      while (!hasPlayableCard(top)) {
      
         System.out.print(SELECT_CARD_FROM_HAND_PROMPT);
         int card2Play = input.nextInt();
      
         if (card2Play < 0 || card2Play > hand.size()) {
      ...
      while (true) {
         System.out.println(SELECT_CARD_FROM_HAND_PROMPT);
         int card2Play = input.nextInt();
         ...
         Card card = hand.getCard(card2Play - 1);
      
         if (!crazyEight.cardMatches(card, top)) {
            System.out.println(CARD_DOES_NOT_MATCH_MESSAGE);
            continue;
         }
      
      
         hand.discard(card2Play - 1);
